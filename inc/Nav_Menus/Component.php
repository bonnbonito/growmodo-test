<?php
/**
 * Bonn\GrowModo\Nav_Menus\Component class
 *
...
namespace Bonn\GrowModo\Nav_Menus;
...
use function Bonn\GrowModo\get_config_content;
 ...
 * * `bonn_growmodo()->is_primary_nav_menu_active()`
 * * `bonn_growmodo()->display_primary_nav_menu( array $args = array() )`
 */
class Component implements Component_Interface, Templating_Component_Interface {
	 ...
	 * - 'bonn_growmodo_dropdown_icon_svg' - Filter the dropdown arrow icon used in navigation menus
	 * - 'bonn_growmodo_menu_toggle_icon_svg' - Filter the hamburger menu icon
	 * - 'bonn_growmodo_menu_close_icon_svg' - Filter the close (X) icon for the mobile menu
	 */
	private function preload_svg_assets() {
		// Load dropdown symbol SVG.
		$dropdown_svg = bonn_growmodo()->wprig_icon( 'dropdown-symbol' );
		 ...
		 */
		$this->dropdown_symbol_svg = apply_filters( 'bonn_growmodo_dropdown_icon_svg', $dropdown_svg );

		// Load menu toggle icons.
		$menu_icon_svg  = bonn_growmodo()->wprig_icon( 'menu-icon' );
		$close_icon_svg = bonn_growmodo()->wprig_icon( 'close-icon' );
		 ...
		 */
		$this->menu_icon_svg = apply_filters( 'bonn_growmodo_menu_toggle_icon_svg', $menu_icon_svg );
		 ...
		 */
		$this->close_icon_svg = apply_filters( 'bonn_growmodo_menu_close_icon_svg', $close_icon_svg );
	}
	...
	public function hooks() {
		...
		add_filter( 'bonn_growmodo_menu_toggle_button', array( $this, 'customize_mobile_menu_toggle' ) );
		add_filter( 'bonn_growmodo_site_navigation_classes', array( $this, 'customize_mobile_menu_nav_classes' ) );
	...
	private function get_theme_settings_config() {
		...
		$this->theme_settings = apply_filters( 'bonn_growmodo_customizer_settings', $theme_settings );
		return null;
	}
	...
	public function action_register_nav_menus() {
		register_nav_menus(
			array(
				static::PRIMARY_NAV_MENU_SLUG => esc_html__( 'Primary', 'bonn-growmodo' ),
			)
		);
	}
	...
	public function filter_primary_nav_menu_dropdown_symbol( string $item_output, WP_Post $item, int $depth, object $args ): string {
		...
		if ( ! empty( $item->classes ) && in_array( 'menu-item-has-children', $item->classes, true ) ) {
			return $item_output . '<button class="dropdown-toggle" aria-expanded="false" aria-label="' . esc_html__( 'Expand child menu', 'bonn-growmodo' ) . '">' . $this->dropdown_symbol_svg . '</button>';
		}
	...
	public function customize_mobile_menu_toggle() {
		return '<button class="menu-toggle icon" aria-label="' . esc_html__( 'Open menu', 'bonn-growmodo' ) . '" aria-controls="primary-menu" aria-expanded="false">
					' . $this->menu_icon_svg . '
					' . $this->close_icon_svg . '
					</button>';
	}
