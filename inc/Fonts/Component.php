<?php
/**
 * Bonn\GrowModo\Fonts\Component class
 *
 * @package bonn_growmodo
 */

namespace Bonn\GrowModo\Fonts;
...
class Component implements Component_Interface, Templating_Component_Interface, Asset_Provider {
	...
	public function get_asset_manifest(): array {
		...
		if ( file_exists( get_template_directory() . '/' . $css_dir . '/google-fonts.css' ) || file_exists( get_template_directory() . '/' . $css_dir . '/google-fonts.min.css' ) ) {
			$manifest['styles']['bonn-growmodo-fonts'] = array(
	...
	public function wprig_register_fonts(): void {
		if ( function_exists( 'wp_register_font_collection' ) ) {
			wp_register_font_collection(
				'modern-stacks',
				array(
					'name'          => __( 'Modern Stacks', 'bonn-growmodo' ),
					'description'   => __( 'A collection of modern system fonts.', 'bonn-growmodo' ),
					'font_families' => array(
						array(
							'font_family_settings' => array(
								'fontFamily' => 'system-ui, sans-serif',
								'slug'       => 'system-ui',
								'name'       => __( 'System UI', 'bonn-growmodo' ),
							),
						...
						array(
							'font_family_settings' => array(
								...
								'slug'       => 'transitional',
								'name'       => __( 'Transitional', 'bonn-growmodo' ),
							),
						...
						array(
							'font_family_settings' => array(
								...
								'name'       => __( 'Monospace Slab Serif', 'bonn-growmodo' ),
							),
						...
						array(
							'font_family_settings' => array(
								...
								'slug'       => 'handwritten',
								'name'       => __( 'Handwritten', 'bonn-growmodo' ),
							),
					...
					'categories'    => array(
						array(
							'name' => __( 'Handwriting', 'bonn-growmodo' ),
							'slug' => 'handwriting',
						),
						array(
							'name' => __( 'Monospace', 'bonn-growmodo' ),
							'slug' => 'monospace',
						),
						array(
							'name' => __( 'Sans Serif', 'bonn-growmodo' ),
							'slug' => 'sans-serif',
						),
						array(
							'name' => __( 'Serif', 'bonn-growmodo' ),
							'slug' => 'serif',
						),
					),
				)
			...
			wp_register_font_collection(
				'local-fonts',
				array(
					'name'          => __( 'Local Fonts', 'bonn-growmodo' ),
					'font_families' => array(
						array(
	...
	public function action_enqueue_fonts(): void {
		...
		if ( wp_style_is( 'bonn-growmodo-fonts', 'registered' ) ) {
			wp_enqueue_style( 'bonn-growmodo-fonts' );
			return;
		}
		...
		if ( '' !== $google_fonts_url && '0' !== $google_fonts_url ) {
			wp_enqueue_style( 'bonn-growmodo-fonts', $google_fonts_url, array(), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
	}
	...
	public function filter_resource_hints( array $urls, string $relation_type ): array {
		if ( 'preconnect' === $relation_type && wp_style_is( 'bonn-growmodo-fonts', 'queue' ) ) {
			$urls[] = array(
		...
		if ( 'preload' === $relation_type ) {
			...
			$fonts_to_preload = apply_filters( 'bonn_growmodo_preload_fonts', $fonts_to_preload );
	...
	protected function get_google_fonts(): array {
		 ...
		 */
		$this->google_fonts = (array) apply_filters( 'bonn_growmodo_google_fonts', $google_fonts );
	...
	protected function get_google_fonts_url(): string {
		...
		$query_args = array(
			...
			'display' => apply_filters( 'bonn_growmodo_google_fonts_display', 'block' ),
		);
	...
	public function download_google_fonts_to_local( array $fonts = array(), string $font_dir = 'assets/fonts', string $css_dir = 'assets/css/src' ): WP_Error|string {
		...
		$google_fonts_url = $google_fonts_base_url . implode( '&', $query_fonts ) . '&display=' . apply_filters( 'bonn_growmodo_google_fonts_display', 'block' );
